...
def edit():
	...
	select_box = Entry(admin_records, width=10)
...
def search_now():
	...
	searched_label = Label(search_records, text=result)
...
# dropdown box
drop = ttk.Combobox(search_records, value=["Search By...", "Unit Type", "Fault"])
drop.current(0)
...
def query():
	...
	# entry fields
	unit = Entry(admin_records, width=40)
	unit.grid(row=0, column=1, sticky=W, padx=20, pady=10)

	fault = Entry(admin_records, width=40)
	fault.grid(row=1, column=1, sticky=W, padx=20, pady=10)

	fix = Entry(admin_records, width=100)
...
# create or connect to database
	...
	admin_records = Tk()
	admin_records.title("Repair Procedures")
	admin_records.geometry("750x500")
...
# c = con.cursor()
	...
	# create field labels
	unit_label = Label(admin_records, text="Unit Type")
	unit_label.grid(row=0, column=0, sticky=E, pady=10, padx=10)

	fault_label = Label(admin_records, text="Fault")
	fault_label.grid(row=1, column=0, sticky=E, pady=10)

	fix_label = Label(admin_records, text="Repair")
	fix_label.grid(row=2, column=0, sticky=E,)

	select_label = Label(admin_records, text="Select Record Number")
	select_label.grid(row=6, column=1)

	# create submit button
	submit_btn = Button(admin_records, text="Submit Record", command=submit, bg="grey")
	submit_btn.grid(row=4, column=0, columnspan=2, pady=10, padx=10, ipadx=30)

	# create a query button
	query_btn = Button(admin_records, text="Show Records", command=query, bg="grey")
	query_btn.grid(row=5, column=0, columnspan=2, pady=10, padx=10, ipadx=30)

	# create a delete button
	delete_btn = Button(admin_records, text="Delete Record", command=delete, bg="grey")
	delete_btn.grid(row=9, column=0, columnspan=2, pady=10, padx=10, ipadx=30)

	# create a Update button
	update_btn = Button(admin_records, text="Edit Record", command=edit, bg="grey")
	update_btn.grid(row=10, column=0, columnspan=2, pady=10, padx=10, ipadx=30)

# create search button
	search_btn = Button(admin_records, text="Search", command=search_records)
...
search_btn.grid(row=0, column=0, padx=10, pady=10, ipadx=30)

button_quit = Button(root, text="EXIT", command=root.quit, fg="white", bg="blue")
button_quit.grid(row=99, column=1)

admin_btn = Button(root, text="Admin", command=admin_records)
admin_btn.grid(row=0, column=1, padx=10, pady=10, ipadx=30)
